...
class Elder(commands.Cog):
    ...
    async def giphy(self, ctx, gif_text):
        logger.info("Peanut butter anyone?")
        if ctx.author.is_on_mobile():
            print(bot_log(ctx.command,gif_text,ctx.author,ctx.channel))
        ...
        if authorized(ctx.author.roles):
            ...
            if fetched is not None:
                ...
                if found == 1:
                    ...
                    guild = ctx.bot.get_guild(settings['discord']['oakGuildId'])
                    is_user, user = isDiscordUser(guild, int(discordId))
                    # else for is_user
        ...
        if authorized(ctx.author.roles):
            ...
            if player == "list" or player is None:
                ...
                if str(reaction.emoji) == reactions[1]:
                    ...
                    await sent_msg.edit(content="Something has gone horribly wrong and you're going to have to talk to <@251150854571163648> about it.  Sorry. :frowning2: ")
                    return
                ...
                await sent_msg.edit(content=f"Warning **{fetched['warning']}** removed for **{fetched['playerName']}**.")
            else:
