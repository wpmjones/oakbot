import discord, pymssql, requests
from discord.ext import commands
from datetime import datetime, timedelta
from config import settings, colorPick, botLog, emojis
...
class Elder:
  ...
  async def elder(self, ctx, command: str = 'help'):
    ...
    if authorized(ctx.author.roles):
      embed = discord.Embed(title = 'Reddit Oak Elder Help Menu', description = "All the elder commands you need but can't remember how to use!", color = colorPick(66,134,244))
      ...
      print(botLog(ctx.command,command,ctx.author,ctx.channel,1))
  ...
  async def giphy(self, ctx, gifText):
    if ctx.author.is_on_mobile():
      print(botLog(ctx.command,gifText,ctx.author,ctx.channel))
  ...
  async def role(self, ctx, player, roleName):
    ...
    if authorized(ctx.author.roles):
      ...
      print(botLog(ctx.command,player,ctx.author,ctx.channel,1))
      await ctx.send("Wait a minute punk! You aren't allowed to use that command")
    ...
    if authorized(ctx.author.roles):
      ...
      if fetched is not None:
        ...
        if found == 1:
          ...
          print(botLog(ctx.command,player,ctx.author,ctx.channel))
  ...
  async def warn(self, ctx, player: str = 'list', *warning):
    ...
    if authorized(ctx.author.roles):
      ...
      if player == 'list' or player is None:
        ...
        embed = discord.Embed(title = 'Reddit Oak Watchlist', description = 'All warnings expire after 60 days.', color = colorPick(181,0,0))
        strikeList = ''
        ...
        print(botLog('list','warnings',ctx.author,ctx.channel))
        await ctx.send(embed=embed)
        return
      elif player == 'remove':
        ...
        print(botLog('Warning Removal',f"Warning: {fetched['warning']}. Player: {fetched['playerName']}",ctx.author,ctx.channel))
        ...
        if fetched is not None:
          ...
          print(botLog('Add warning',player,ctx.author,ctx.channel))
  ...
  async def unconfirmed(self, ctx, *args):
    ...
    if authorized(ctx.author.roles):
      ...
      if arg == 'list':
        ...
        print(botLog(ctx.command,arg,ctx.author,ctx.channel,1))
        content = 'You have provided an invalid arguement. Please specify `list`, `kick`, or `move`.'
        return
      print(botLog(ctx.command,arg,ctx.author,ctx.channel))
      await ctx.send(content)
    else:
      print(botLog(ctx.command, args[0],ctx.author,ctx.channel,1))
      await ctx.send("Wait a minute punk! You aren't allowed to use that command")
  ...
  async def presence(self, ctx, *, msg: str = 'x'):
    ...
    if authorized(ctx.author.roles):
      ...
      print(botLog(ctx.command,msg,ctx.author,ctx.channel,1))
