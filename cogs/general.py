...
from config import settings, emojis, colorPick, botLog
...
class General:
  ...
  async def on_message(self, message):
    ...
    if 'rules' in message.content:
      await message.channel.send('Reddit Oak rules can be found here - https://www.reddit.com/r/CoC_RedditOak/wiki/moderation/general_rules')
      return
    if any(word in message.content for word in ['signup','sign-up','sign up']):
      await message.channel.send('Please use `/war c#` or `/war call #` to call targets in <#' + str(settings['oakChannels']['cocChat']) + '> or <#' + str(settings['oakChannels']['oakWar']) + '>')
      return
  ...
  async def player(self, ctx, *, playerName: str = 'x'):
    ...
    if playerName == 'x':
      print(botLog(ctx.command,'no player name',ctx.author,ctx.channel))
    ...
    try:
      row = cursor.fetchone()
      if row is None:
        print(botLog(ctx.command,'player not found in database',ctx.author,ctx.channel))
        await ctx.send(f"{emojis['other']['redx']} The player you provided was not found in the database.  Please try again.")
        return
    except:
      print(botLog(ctx.command,f'other error - {arg}',ctx.author,ctx.channel))
    ...
    embed = discord.Embed(title = f"{emojis['league'][row['leagueEmoji'][1:-1]]} {row['playerName'].strip()} ({row['tag'].strip()})", color = colorPick(226,226,26))
    embed.add_field(name = 'Town Hall', value = f"{emojis['thIcon'][row['thLevel']]} {str(row['thLevel'])}", inline = True)
    ...
    embed.add_field(name = 'Builder Hall Level', value = f"{emojis['bhIcon'][row['builderHall']]} {str(row['builderHall'])}", inline = False)
    ...
    print(botLog(ctx.command,playerName,ctx.author,ctx.channel))
  ...
  async def help(self, ctx, command: str = 'all'):
    ...
    if command not in ['all','siege','player','elder']:
      print(botLog(ctx.command,f'{command} is an invalid option',ctx.author,ctx.channel))
      await ctx.send(f"{emojis['other']['redx']} You have provided a command that does not exist.  Perhaps try /help to see all commands.")
      return
    embed = discord.Embed(title = 'The Arborist by Reddit Oak', description = desc, color = colorPick(15,250,15))
    ...
    embed.set_footer(icon_url = 'https://openclipart.org/image/300px/svg_to_png/122449/1298569779.png', text = 'The Arborist proudly maintained by TubaKid.')
    print(botLog('help',command,ctx.author,ctx.channel))
    await ctx.send(embed=embed)

  @commands.command(name='war', hidden=True)
  async def warHandler(self, ctx):
    print('Skipping war command')
    pass

  @commands.command(name='siege', aliases=['sm'])
  async def siegeRequest(self, ctx, *, type: str = 'help'):
    ...
    if type == 'help':
      embed = discord.Embed(title = 'The Arborist by Reddit Oak', color = colorPick(15,250,15))
      embed.add_field(name = 'Commands:', value = '-----------', inline = True)
      siege = ('Posts request for the specified siege machine in Discord and tags those players that can donate.'
      '\n**ground**: Wall Wrecker'
      '\n**blimp**: Battle Blimp'
    ...
    donors = []
    for row in fetched:
      donors.append(row['playerName'].rstrip() + ': <@' + row['slackId'] + '>')
    embed = discord.Embed(title = siegeName + ' Request', description = requestor + ' has requested a ' + siegeName, color=0xb5000)
    ...
    content += '\n'.join(donors)
    print(botLog(ctx.command,siegeType,ctx.author,ctx.channel))
    await ctx.send(embed=embed)
