...
class General:
    ...
    async def on_message(self, message):
        ...
        if any(word in message.content for word in ["signup","sign-up","sign up"]):
            await message.channel.send(f"Please use `/war c#` or `/war call #` to call targets in <#{str(settings['oakChannels']['cocChat'])}> or <#{str(settings['oakChannels']['oakWar'])}>")
            return
    ...
    async def player(self, ctx, *, playerName: str = "x"):
        ...
        try:
            row = cursor.fetchone()
            if row is None:
                print(bot_log(ctx.command,"player not found in database",ctx.author,ctx.channel))
        ...
        embed = discord.Embed(title = f"{emojis['league'][row['leagueEmoji'][1:-1]]} {row['playerName'].strip()} ({row['tag'].strip()})", color = color_pick(226,226,26))
    ...
    async def help(self, ctx, command: str = "all"):
        ...
        if command not in ["all","siege","player","elder"]:
            print(bot_log(ctx.command,f"{command} is an invalid option",ctx.author,ctx.channel))
        ...
        print(bot_log("help",command,ctx.author,ctx.channel))
        await ctx.send(embed=embed)
    ...
    async def siegeRequest(self, ctx, *, type: str = "help"):
        ...
        if type == "help":
            ...
            embed.add_field(name = "/siege <siege type>", value = siege, inline = False)
            embed.set_footer(icon_url = "https://openclipart.org/image/300px/svg_to_png/122449/1298569779.png", text = "The Arborist proudly maintained by TubaKid.")
            print(bot_log("help","siege (issued through /siege)",ctx.author,ctx.channel))
            await ctx.send(embed=embed)
            return
